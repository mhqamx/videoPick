...
iOS 抖音视频下载 Demo，采用"iOS 客户端 + Python backend 解析代理"双模式架构。iOS 端优先调用后端，后端不可用时自动回退到本地 HTML 解析。
...
```
FastAPI (main.py)
  → ExtractorRegistry → [DouyinExtractor, ...] → local_resolver.py
```
...
**扩展新平台：** 在 `backend/app/extractors/` 新建文件实现 `VideoExtractor` Protocol → 在 `registry.py` 注册。

## 重要实现细节

- **去水印：** `/playwm/` 替换为 `/play/`，移除 `logo_name` 和 `watermark` 查询参数
...
- **抖音页面结构会变化：** 解析逻辑需要跟进抖音前端变更，调试时用 `curl -L <url> -H 'User-Agent: Mozilla/5.0 (iPhone; ...)'` 抓取页面检查结构
